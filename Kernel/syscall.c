...
{ 
  ...
  case WRITELCDN:
    ...
    RPI_GetGpio()->GPSET0 = (1 << 16); // liga led
    break;
  case WRITELCDS:
    /* Nao foi implementado */
    RPI_GetGpio()->GPCLR0 = (1 << 16); // desliga led
    break;
  case EXITTASK:
    sys_taskexit();
    break;
...
void sys_taskcreate(int *ID,void (*task)())
{
  ...
  *ID=NumberTaskAdd;
  ...
  Descriptors[NumberTaskAdd].SP=&Descriptors[NumberTaskAdd].Stack[SizeTaskStack-10];
  return;
}
...
void sys_ligaled()
{
  RPI_GetGpio()->GPSET0 = (1 << 16); // GPIO16 pino 36
}

void sys_desligaled()
{
  RPI_GetGpio()->GPCLR0 = (1 << 16); // GPIO16 pino 36
}
...
void sys_nkread(char *tipo, void *value)// endereço da variavel que recebe o dado solicitado será armazenado em serial_queue[posicao_a_inserir].value
{
  ...
  serial_queue[posicao_a_inserir].tid=TaskRunning;
	if(!strcmp(tipo,"%f"))//{//28/02/15 
	   serial_queue[posicao_a_inserir].real=(void *)value; ///printk("value = real\n");}// value é o endereço da variável que recebe o dado solicitado na app
  ...
  Descriptors[TaskRunning].State=BLOCKED;// RPI_GetGpio()->GPSET0 = (1 << 16); // acendeu
  //printk("Tsk %d está bloqueada aguardando dado\n",TaskRunning);
  posicao_a_inserir++;  	
